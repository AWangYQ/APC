    ...
    def __call__(self, frame_indices):
        ...
        if len(frame_indices) >= self.size * self.stride:
            ...
            for i in range(self.size):
                clips.append(
                    frame_indices[len(frame_indices) // self.size * i: len(frame_indices) // self.size * (i + 1)])
            out = []
            for i in range(self.size):
...
class DenseCrop(object):
    ...
    def __call__(self, frame_indices):
        ...
        n = len(frame_indices)
        clips = []
        interval = n // self.size
        if n > self.size:
            for index in range(interval):
                clips.append(frame_indices[index: index + interval * self.size:interval])
        else:
            clip = list(frame_indices[0:])
            for index in clip:

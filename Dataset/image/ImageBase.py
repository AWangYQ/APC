...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #if 2*h < w:
        #    pad_pixel = int((w - 2 * h)/4)
        #    pad = Pad((pad_pixel, 0, pad_pixel, 0))
        #    img = pad(img)
        #elif 2*h > w:
        #    pad_pixel = int((2*h - w)/2)
        #    pad = Pad((0, pad_pixel, 0, pad_pixel))
        #    img = pad(img)
        if self.transform is not None:
            img = self.transform(img)

        return img, pid, camid, trackid, img_path.split('/')[-1]   # 迭代器的输出格式 图片向量；图片标签；相机标签；轨迹标签（感觉可以不要）；图片名字（感觉需要改成图片路径）

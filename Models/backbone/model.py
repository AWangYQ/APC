...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        sel.conv2 = nn.Conv2d(planes, planes, 3, padding=1, bias=False)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        #self.experts = nn.ModuleList([ExpertModel(d_model, d_model) for _ in range(8)])
        #self.gating = GatingMechanism(d_model, 8)
    ...
    def forward(self, x: torch.Tensor, adapter: torch.nn.Module=None):
        x = x + self.attention(self.ln_1(x))
        if adapter is not None:
            x = x + self.mlp(self.ln_2(x)) + 0.2 * adapter(x)
        else:
            #expert_outputs = [expert(x) for expert in self.experts]
            #expert_outputs = torch.stack(expert_outputs, 2)
            #gating_weights = self.gating(x)
            #expert_x = torch.sum(expert_outputs * gating_weights.unsqueeze(-1), 2)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None, adapter=None, prompts=None):
        ...
        try:
            if cv_emb != None:
                x[:,0] = x[:,0] + cv_emb   # 这里只将sie embedding 加到了cls中
        except:
            ipdb.set_trace()
        ...
        for i in range(len(self.transformer.resblocks)):
            if adapter is not None:
                x = self.transformer.resblocks[i](x, adapter[i])
            else:
